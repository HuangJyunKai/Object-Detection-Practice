...
def resize(image, boxes, dims=(352, 352), return_percent_coords=True):
    """
...
def transform(image, boxes, labels, split):
    ...
    new_image, new_boxes = resize(new_image, new_boxes, dims=(352, 352))
...
def save_checkpoint(epoch, model, optimizer):
    ...
    state = {'epoch': epoch,
             'model': model,
             'optimizer': optimizer}
    filename = 'checkpoint_ssd352.pth.tar'
    torch.save(state, filename)

def save_bestcheckpoint(epoch, model, optimizer):
    """
    Save model checkpoint.
